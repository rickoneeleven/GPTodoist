...
def create_task_id_prompt(user_message):
    ...
    prompt = f"{user_message}. What was the task ID of that task? Respond with [Task ID: X], in that format exactly, including the square brackets. never fabricate a task id. Then have a read of our conversation so far and ask me a context sensitive question about my weight."
    return prompt
